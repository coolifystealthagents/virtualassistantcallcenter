...
const guides: Record<string, { answer: string; sections: { title: string; text: string; bullets?: string[] }[]; faq: { q: string; a: string }[] }> = {
  'virtual-assistant-planning': {
    answer: 'Start VA phone support with one call queue and a limited block of coverage. Once the script works and call volume is steady, you can decide whether a larger weekly schedule makes sense.',
    sections: [
      { title: 'What changes the plan', text: 'The scope grows when the assistant handles live calls, books appointments, updates the CRM, covers weekends, or needs a manager to review call quality.', bullets: ['Live answering needs more training and review than back-office admin.', 'Nights, weekends, and split shifts need a clear coverage schedule.', 'Managed support may include screening, backup coverage, and QA. Ask what is included.'] },
  ...
  'tasks-to-delegate-first': {
    answer: 'Delegate calls that are common, repeatable, and easy to check. Keep angry customers, refunds, legal or medical questions, and policy exceptions with a manager at first.',
    sections: [
