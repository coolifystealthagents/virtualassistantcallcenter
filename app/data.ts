...
import featuredImages from '../content/featured-images.json';

const imageBySlug = Object.fromEntries(featuredImages.map((item) => [item.slug, item]));
...
].map((post) => ({ ...post, ...imageBySlug[post.slug] }));
