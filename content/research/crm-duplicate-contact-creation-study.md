---
slug: crm-duplicate-contact-creation-study
title: Does phone intake avoid creating duplicate CRM contacts?
description: A reproducible study of contact search, match confidence, record creation, merge review, ownership, and correction in phone intake.
datePublished: 2026-09-23
published: 2026-09-23
updated: 2026-09-23
category: CRM research
image: /thumbnails/crm-duplicate-contact-creation-study.svg
imageAlt: CRM duplicate study connecting caller details, search candidates, match confidence, create-or-review decision, and merge history
related: /services/qa-crm, /research, /contact
---

## Research question

Does phone intake search and match existing CRM records well enough to avoid duplicate contacts without attaching new information to the wrong person?

Creating a new record for every uncertain caller fragments history, while forcing a weak match can disclose or overwrite another person’s information. Shared phone numbers, changed email addresses, spelling variation, and incomplete legacy data make exact matching unreliable. This is a control problem, not a reason for a receptionist to improvise. The business owner must define what may be collected, what counts as sufficient evidence, when work must pause, and who can resolve an exception. A Philippines-based virtual assistant can follow those rules, document the result, and route uncertainty; the assistant should not convert a weak signal into a confident operational decision.
...
For each eligible call, preserve the normalized search inputs, candidate records returned, match fields, confidence band, disqualifying conflict, create-or-review decision, assigned owner, source call token, merge request, merge approval, surviving record, and correction history. The evidence should connect the call event to the downstream object with stable identifiers. A free-text note without a link to the final record is not enough, because a reviewer cannot tell whether the note and the action describe the same case.

Use explicit workflow states: unique confident match; no candidate; multiple candidates; conflicting identifier; provisional new contact; owner review required. Each state needs an owner, permitted next action, and deadline where timing matters. Do not let a blank field mean both “not asked” and “asked but unknown.” Do not treat an automation success message as proof that a human decision occurred.
...
The primary measure is correct create-or-link decision rate. Define its numerator and denominator before inspection. Report a separate insufficient-evidence rate; do not remove missing cases silently or count them as correct. Show both counts and percentages, because a high percentage based on a small sample can mislead.

Secondary measures include duplicate creation, wrong-person linkage, unresolved candidate rate, manual review time, merge reversal, orphaned activity, and repeat-contact continuity. Segment cautiously by workflow version, call type, shift, destination system, and exception state when those cuts answer a declared operational question. Avoid ranking individual workers from small samples. A cluster around one script version or integration release often says more than a leaderboard.
