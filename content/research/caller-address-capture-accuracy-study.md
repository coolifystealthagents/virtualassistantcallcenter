---
slug: caller-address-capture-accuracy-study
title: Does phone intake capture a service address accurately enough to act on?
description: A reproducible study of address capture, read-back, geocoding boundaries, corrections, and routing evidence in phone intake.
datePublished: 2026-09-23
published: 2026-09-23
updated: 2026-09-23
category: Call intake research
image: /thumbnails/caller-address-capture-accuracy-study.svg
imageAlt: Phone intake address study linking caller wording, read-back, normalized fields, owner review, and correction
related: /services/call-answering, /research, /contact
---

## Research question

Does phone intake preserve a caller’s service address accurately enough for an approved team to route, schedule, or follow up without guessing?

A plausible-looking address can still point to the wrong unit, municipality, entrance, or service area. Autocomplete may normalize the wrong place, accents and line quality can change street names, and a caller may give a mailing address when the workflow needs a service location. This is a control problem, not a reason for a receptionist to improvise. The business owner must define what may be collected, what counts as sufficient evidence, when work must pause, and who can resolve an exception. A Philippines-based virtual assistant can follow those rules, document the result, and route uncertainty; the assistant should not convert a weak signal into a confident operational decision.
...
For each eligible call, preserve the caller’s verbatim address statement, address type, unit or suite, locality, region, postal code, landmark when authorized, read-back result, normalization candidate, confidence or ambiguity flag, service-area decision, reviewer action, and every correction. The evidence should connect the call event to the downstream object with stable identifiers. A free-text note without a link to the final record is not enough, because a reviewer cannot tell whether the note and the action describe the same case.

Use explicit workflow states: complete and confirmed; incomplete; multiple candidates; outside declared coverage; caller declined; owner review required. Each state needs an owner, permitted next action, and deadline where timing matters. Do not let a blank field mean both “not asked” and “asked but unknown.” Do not treat an automation success message as proof that a human decision occurred.
...
The primary measure is exact confirmed-address rate. Define its numerator and denominator before inspection. Report a separate insufficient-evidence rate; do not remove missing cases silently or count them as correct. Show both counts and percentages, because a high percentage based on a small sample can mislead.

Secondary measures include missing-unit rate, locality disagreement, postal-code disagreement, normalization override, read-back correction, service-area false acceptance, service-area false rejection, and downstream reroute. Segment cautiously by workflow version, call type, shift, destination system, and exception state when those cuts answer a declared operational question. Avoid ranking individual workers from small samples. A cluster around one script version or integration release often says more than a leaderboard.
