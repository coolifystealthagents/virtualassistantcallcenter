---
slug: appointment-slot-availability-integrity
...
related: appointment-booking-no-show-research
---

## Headline finding

Availability is reliable only when the slot described to a caller, the slot held by the calendar, and the slot finally confirmed belong to the same rule set. Define a cohort by service, location, duration, staff qualification, time zone, and review period. Sample offered slots, declined offers, completed bookings, and calls that ended without a slot. Preserve request time, source of availability, rule version, offer, hold state, response, and final calendar result. Report offer accuracy, booking completion, later correction, and conflict counts with separate denominators. A free slot does not prove deliverability, attendance, or satisfaction. Calendar exports may omit manual holds and cancelled events, while short samples cannot establish a stable benchmark. Review the transition from lookup to hold to confirmation so concurrency problems are not confused with bad service rules. State exclusions and missing evidence. The conclusion should distinguish inventory truth from caller choice and later scheduling change.
