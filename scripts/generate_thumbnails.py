...
def main():
    ...
    requested = {slug for slug in os.environ.get("THUMBNAIL_SLUGS", "").split(",") if slug}
    entries = []
    for path in sorted((ROOT / "content").glob("*/*.md")):
        ...
        if meta.get("slug") and meta.get("title") and (not requested or meta["slug"] in requested):
            entries.append((str(path.relative_to(ROOT)), meta))
    manifest = ROOT / "content" / "featured-images.json"
    for meta in json.loads(manifest.read_text(encoding="utf-8")):
        if not requested or meta.get("slug") in requested:
            entries.append((str(manifest.relative_to(ROOT)), meta))
    for source, meta in entries:
        ...
        print(f"{source} -> {target.relative_to(ROOT)} ({BACKGROUNDS[index].name})")
